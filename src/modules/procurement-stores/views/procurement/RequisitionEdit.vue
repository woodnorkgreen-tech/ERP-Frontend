<template>
  <div class="space-y-6">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li>
          <div class="flex items-center">
            ...
            <router-link :to="`/procurement/requisition/${id}`" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">
              Details
            </router-link>
          </div>
        </li>
    ...
    <!-- Header -->
    <div>
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Requisition</h1>
      <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update requisition information</p>
    </div>
    ...
    <div v-if="loadingData" class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-8">
      <div class="flex items-center justify-center">
        ...
        <p class="ml-3 text-gray-600 dark:text-gray-400">Loading requisition data...</p>
      </div>
    </div>
...
const fetchRequisition = async () => {
  ...
  try {
    ...
    console.error('Failed to fetch requisition:', e)
    errorMessage.value = 'Failed to load requisition data'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.put(`/api/procurement-stores/requisitions/${id}`, payload)
    ...
    if (resData.success === false) {
      if (resData.error && typeof resData.error !== 'string') {
        ...
        errorMessage.value = resData.error || resData.message || 'Failed to update requisition'
      }
      loading.value = false
      return
    }
    
    const newId = resData.id || resData.data?.id || id
    router.push(`/procurement/requisition/${newId}`)
  } catch (error: any) {
    ...
    if (data?.error) {
      ...
      errorMessage.value = 'An error occurred while updating the requisition.'
    }
  } finally {
    loading.value = false
  }
