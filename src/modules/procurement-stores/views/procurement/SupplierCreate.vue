<template>
  <div class="space-y-6">
      ...
      <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li aria-current="page">
          <div class="flex items-center">
            ...
            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Add Supplier</span>
          </div>
        </li>
      </ol>
    </nav>
    ...
    <!-- Header -->
    <div>
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Add New Supplier</h1>
      <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Create a new supplier record</p>
    </div>
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.post('/api/procurement-stores/suppliers', formData.value)
    ...
    if (error.response?.data?.errors) {
      ...
      errorMessage.value = 'An error occurred while saving the supplier.'
    }
  } finally {
    loading.value = false
  }
