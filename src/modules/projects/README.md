...
## 5. Global Toggle Architecture (Internal Category)

To maintain a clean and unified user interface, the Projects Dashboard avoids dedicated tabs for specialized project types (like Internal Jobs or Sponsorships). Instead, it implements a **Global Toggle Pattern**.

### 🏗️ Architectural Concept
Traditionally, specialized categories often lead to "Parallel Dashboards" where code is duplicated to handle the same status transitions (e.g., Internal Pipeline vs External Pipeline). 

The WNG-ERP resolves this by:
1.  **Uniform Lifecycle**: All project types (Commercial, Internal, Sponsorship) share the same lifecycle statuses (`enquiry_logged` → `quote_approved` → `completed`).
2.  **Global Property Filter**: The `ProjectsEnquiries.vue` component maintains an `is_non_profit` state flag in its `filters` object.
3.  **Cross-Category Navigation**:
    *   The primary tabs (**New Enquiries**, **In Progress**, **Completed**) define the **Phase** of the project.
    *   The **Internal & Sponsorships** toggle defines the **Nature** of the project.
4.  **Backend Decoupling**: The `EnquiryController.php` processes current view (Phase) and the toggle (Nature) as independent query filters. This allows internal jobs to move across the dashboard as they progress, just like standard records.

### 🎨 Frontend Adaptive UI
- When the internal toggle is active, the system automatically adjusts:
    - **Labels**: "All History" becomes "Finished Internal".
    - **KPI/Counts**: Derived based on the current phase AND the internal flag.
    - **Status Options**: The status dropdown filters out irrelevant options (e.g., "New Client" for internal jobs).

### ✅ Benefits
- **Zero Redundancy**: No separate components or routers needed for internal jobs.
- **Consistent UX**: Users interact with the same board layouts regardless of the job type.
- **Scalability**: New specialized categories can be added as simple flags without bloating the primary navigation.
