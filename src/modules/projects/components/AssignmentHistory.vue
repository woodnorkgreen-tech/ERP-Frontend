    ...
    <div class="bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl w-full max-w-2xl h-[85vh] flex flex-col relative z-20 border border-white/20 dark:border-gray-800 overflow-hidden animate-in fade-in zoom-in duration-300">
      ...
      <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-white to-gray-50 dark:from-gray-900 dark:to-gray-800/40 relative">
        <div class="flex items-center justify-between gap-6">
          <div class="flex items-center space-x-6 min-w-0">
             <div class="p-3 bg-blue-500/10 rounded-2xl shadow-inner border border-blue-500/20">
              <i class="mdi mdi-timeline-clock-outline text-blue-600 text-2xl"></i>
            </div>
            <div class="min-w-0">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar bg-gray-50/30 dark:bg-gray-900/40 p-8">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-20 space-y-4">
          ...
          <p class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">Synchronizing Intel...</p>
        </div>

        <!-- Error Alert -->
        <div v-else-if="error" class="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 flex items-start space-x-4">
          <div class="p-2 bg-red-100 dark:bg-red-900/30 rounded-lg">
            <i class="mdi mdi-alert-decagram text-red-600 dark:text-red-400 text-xl"></i>
          </div>
          <div>
            <h4 class="text-sm font-black text-red-900 dark:text-red-300 uppercase tracking-widest mb-1">Retrieval Fault</h4>
        ...
        <div v-else>
          <div v-if="assignmentHistory.length === 0" class="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-800/50 rounded-[2rem] border border-dashed border-gray-200 dark:border-gray-700">
            <div class="w-16 h-16 rounded-full bg-gray-50 dark:bg-gray-800 flex items-center justify-center mb-4 opacity-50">
              <i class="mdi mdi-database-off-outline text-gray-400 text-3xl"></i>
            </div>
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">No assignment vectors detected</p>
          </div>
            ...
            >
              ...
              <div class="bg-white dark:bg-gray-800/60 p-6 rounded-[1.5rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div class="space-y-1">
                    <div class="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.15em] mb-1">Personnel Reassignment</div>
                    <div class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">
                      To: <span class="text-blue-600 dark:text-blue-400">{{ history.assignedTo?.name || 'Unknown Intel' }}</span>
                    </div>
                    <div class="text-[11px] text-gray-400 dark:text-gray-500 font-bold">
                      Authored by {{ history.assignedBy?.name || 'Grand Council' }}
                    </div>
                  </div>
                ...
                <div v-if="history.notes" class="mt-4 p-4 bg-gray-50/50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 relative group-hover:bg-blue-500/5 transition-colors">
                  <div class="absolute -top-2 left-4 px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded-full text-[9px] font-black text-gray-400 uppercase tracking-widest border border-gray-200 dark:border-gray-700">Directive</div>
