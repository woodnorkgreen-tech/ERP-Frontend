...
import type { Employee, EmployeeFilters, CreateEmployeeData, UpdateEmployeeData } from '../../../shared/types/employee'
...
export function useEmployees() {
  ...
  const createEmployee = async (employeeData: CreateEmployeeData) => {
    ...
    try {
      ...
      error.value = handleApiError(err, 'Failed to create employee')
      throw err
    } finally {
      loading.value = false
    }
  ...
  const updateEmployee = async (id: number, employeeData: UpdateEmployeeData) => {
    ...
    try {
      ...
      error.value = handleApiError(err, 'Failed to update employee')
      throw err
    } finally {
      loading.value = false
    }
  ...
  const toggleEmployeeStatus = async (id: number) => {
    ...
    try {
      ...
      const response = await put(`/api/hr/employees/${id}`, { status: newStatus } as unknown as Record<string, unknown>)
      const responseData = response.data
      ...
      error.value = handleApiError(err, 'Failed to toggle employee status')
      throw err
    } finally {
      loading.value = false
    }
